...
For other Linux systems, existing archive migrations, or removal of an older
installation that includes `onlybackup-vault`, see [INSTALL.md](INSTALL.md).
...
- a trusted computer on which to keep the deposit credential and age identity;
...
installed. If services or data already exist, do not initialize the state.
Follow the migration or controlled reinstall procedure in
...
On the trusted computer, install at least `onlybackup` and
`onlybackup-recover` from the same release. Create the age identity once:
...
with the public recipient from `age-public.json`:
...
older programs. For a clean reinstall or migration from an older vault-based
architecture, follow [INSTALL.md](INSTALL.md).
